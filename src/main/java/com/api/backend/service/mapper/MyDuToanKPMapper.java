...
@Component
public class MyDuToanKPMapper {

    @Autowired private DetaiRepository detaiRepository;
    ...
    public DutoanKPDTO toDto(DutoanKP dutoanKP) {
        ...
        if(dutoanKP.getDetai() != null) dutoanKPDTO.setDetaiId( dutoanKP.getDetai().getId() );
        ...
        dutoanKPDTO.setTongTien(dutoanKP.getDutoanKPCTS().stream().map(dutoanKPCT -> dutoanKPCT.getTong()).collect(Collectors.summingLong(Long::longValue)));
